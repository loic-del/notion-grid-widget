...
function esc(s = "") {
  return String(s).replace(/[&<>\"']/g, c => ({
    "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;",
  })[c]).slice(0, 5000); // anti crash sur très longues valeurs
}
...
<div class="modal-backdrop" id="backdrop" aria-hidden="true">
  <div class="modal" role="dialog" aria-modal="true">
    <div class="modal-header">
      ...
      <div><span id="m-status" class="pill">Status</span>
      <button class="close" id="m-close" aria-label="Fermer">Fermer</button></div>
    </div>
...
grid?.addEventListener('click', (e)=>{
  const fig = e.target.closest('.item'); if(!fig) return;
  const name = fig.dataset.name || ""; const image = fig.dataset.image || "";
  const status = fig.dataset.status || ""; const date = fig.dataset.date || "";
  const pinned = fig.dataset.pinned === "true"; const desc = fig.dataset.desc || "";
