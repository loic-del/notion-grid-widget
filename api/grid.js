...
export default async function handler(req, res) {
  try {
    ...
    const qPlatforms = String(req.query.platform || req.query.platforms || "").trim();
    const platforms = qPlatforms ? qPlatforms.split(",").map(s => s.trim()).filter(Boolean) : null;

    const items = await fetchPosts({ databaseId, pageSize: size, platforms });
    const html = renderHTML({ items, cols, gap, radius, showCaptions, platforms });
...
function renderHTML({ items, cols, gap, radius, showCaptions, platforms }) {
  const statusStyle = (s) => {
    if (!s) return "";
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .top { display:flex; align-items:center; justify-content:space-between; gap:8px; margin-bottom:var(--gap); }
  .hint { font-size:12px; color:#6b7280; }
  .chips { display:flex; gap:6px; flex-wrap:wrap; }
  .chip { padding:4px 8px; border-radius:999px; font-size:12px; border:1px solid #e5e7eb; background:#fff; }
  ...
  .modal-media { background:#000; aspect-ratio:1/1; display:flex; align-items:center; justify-content:center; }
...
<body>
  <div class="wrap">
    <div class="top">
      <div class="hint">${platforms && platforms.length ? `Filtré: ${platforms.map(esc).join(", ")}` : " "}</div>
      <div class="hint">Astuce: ajouter <code>?platform=Instagram,TikTok</code> à l'URL pour filtrer</div>
    </div>
  ...
  <script>
    ...
    grid?.addEventListener('click', (e)=>{
      ...
      // chips plateformes
      mPlatforms.innerHTML = platforms.map(p => `<span class="chip-sm">${p}</span>`).join("") || "—";
